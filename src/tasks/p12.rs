...
pub fn get_shortest_path_len(filename: &str) -> u32
{
    ...
    'outer: for i in 0..map.len()
    {
        ...
        {
            if map[i].chars().nth(j).unwrap() == 'S'
            { start_pos = (i as i32, j as i32); }
            else if map[i].chars().nth(j).unwrap() == 'E'
            { final_pos = (i as i32, j as i32); }

            if start_pos != (0, 0) && final_pos != (0, 0)
            { break 'outer; }
        }
    }
...
pub fn get_fewest_steps(filename: &str) -> u32
{
    ...
    for start_pos in start_positions
    {
        ...
        if path_len.is_some() && path_len.unwrap() < shortest_path_len
        { shortest_path_len = path_len.unwrap(); }
    }
